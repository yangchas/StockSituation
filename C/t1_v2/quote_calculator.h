...
private:
    ...
    static int limit_band_bp(const RawTick& tick);
    static bool is_20pct_symbol(const char* symbol);
    static bool is_30pct_market(const char* market);
    static bool near_price(int lhs_milli, int rhs_milli);
    static bool has_limit_seal_order(const RawTick& tick, int limit_price_milli, bool upper, MarketPhase phase);
